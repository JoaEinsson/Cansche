<template>
  <div
    ...
    @click.self="close"
  >
    <div class="w-full max-w-4xl h-[min(680px,calc(100vh-2rem))] flex overflow-hidden bg-[#0f1011] border border-[#23252a] rounded-[12px] shadow-design-xl">
      <aside class="w-56 shrink-0 border-r border-[#23252a] bg-[#08090a] p-3 flex flex-col">
        <div class="px-2 py-2 mb-3">
          <div class="flex items-center gap-2 text-white font-semibold text-sm">
            <IconRenderer icon="lucide:Settings" :size="16" color="#e4f222" />
            Configurações
          </div>
        ...
        <nav class="space-y-1">
          <button v-for="item in sections" :key="item.id" @click="activeSection = item.id" class="w-full flex items-center gap-2 px-2.5 py-2 rounded-[6px] text-xs text-left transition-colors" :class="activeSection === item.id ? 'bg-[#161718] text-white border border-[#383b3f]' : 'text-[#8a8f98] hover:bg-[#0f1011] hover:text-white border border-transparent'">
            <IconRenderer :icon="item.icon" :size="14" :color="activeSection === item.id ? '#e4f222' : '#8a8f98'" />
            {{ item.label }}
          </button>
        </nav>
        ...
        <div class="flex-1 overflow-y-auto p-5">
          <section v-if="activeSection === 'general'" class="space-y-4">
            <SettingRow title="Abrir no mês atual" description="Sempre iniciar a aplicação no mês de hoje.">
              <input v-model="draft.general.openToToday" type="checkbox" class="h-4 w-4 accent-[#e4f222]" />
            </SettingRow>
            <SettingRow title="Confirmar ações destrutivas" description="Pedir confirmação antes de excluir calendários, modelos ou eventos.">
              <input v-model="draft.general.confirmDestructiveActions" type="checkbox" class="h-4 w-4 accent-[#e4f222]" />
            </SettingRow>
          </section>
          ...
          <section v-else-if="activeSection === 'calendar'" class="space-y-4">
            ...
            <SettingRow title="Mostrar finais de semana" description="Exibir sábados e domingos normalmente na grade.">
              <input v-model="draft.calendar.showWeekends" type="checkbox" class="h-4 w-4 accent-[#e4f222]" />
            </SettingRow>
            <SettingRow title="Mostrar número da semana" description="Exibir a semana ISO no cabeçalho da grade.">
              <input v-model="draft.calendar.showWeekNumbers" type="checkbox" class="h-4 w-4 accent-[#e4f222]" />
            </SettingRow>
          ...
          <section v-else-if="activeSection === 'updates'" class="space-y-4">
            <div class="p-4 rounded-[6px] border border-[#23252a] bg-[#08090a]">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <div class="flex items-center gap-2 text-white text-sm font-semibold">
                    <IconRenderer icon="lucide:RefreshCw" :size="16" color="#e4f222" :class="updateStatus === 'checking' ? 'animate-spin' : ''" />
              ...
              <div v-else class="mt-4 flex items-center justify-between gap-3">
                <label class="flex items-center gap-2 text-xs text-[#d0d6e0]">
                  <input v-model="draft.updates.autoCheckEnabled" type="checkbox" class="h-4 w-4 accent-[#e4f222]" />
            ...
            <div v-if="updateInfo?.hasUpdate" class="p-4 rounded-[6px] border border-[#e4f222]/50 bg-[#e4f222]/5 space-y-3">
              ...
              <button @click="emit('start-update', progressHandler)" :disabled="updateStatus === 'downloading'" class="px-3 py-1.5 rounded-[6px] text-xs font-medium bg-[#e4f222] text-[#08090a] hover:bg-[#e4f222]/80 disabled:opacity-50">
                {{ updateStatus === 'downloading' ? `Baixando ${downloadPercent}%` : 'Baixar e reiniciar' }}
              </button>
            </div>

            <div v-if="updateError" class="p-3 rounded-[6px] border border-[#eb5757]/50 bg-[#eb5757]/10 text-xs text-[#eb5757]">
              {{ updateError }}
            </div>
          </section>
          ...
          <section v-else-if="activeSection === 'data'" class="space-y-4">
            ...
            <div class="p-4 rounded-[6px] border border-[#eb5757]/30 bg-[#eb5757]/5">
              ...
              <button @click="resetSettings" class="mt-3 px-3 py-1.5 rounded-[6px] text-xs text-[#eb5757] border border-[#eb5757]/50 hover:bg-[#eb5757]/10">Restaurar padrões</button>
            </div>
          </section>

          <section v-else-if="activeSection === 'about'" class="space-y-5">
            <div class="flex items-center gap-4">
              <img src="/cansche-mark.svg" alt="Cansche" class="w-14 h-14 rounded-[12px]" />
              <div>
                ...
                <p class="text-xs text-[#e4f222] font-mono mt-1">v{{ appVersion }}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div v-if="currentRelease" class="space-y-3">
                <div v-for="section in currentRelease.sections" :key="section.title">
                  <h4 class="text-xs font-medium text-[#e4f222]">{{ section.title }}</h4>
...
const statusClass = computed(() => {
  if (props.updateStatus === 'available') return 'bg-[#e4f222]/15 text-[#e4f222]';
  if (props.updateStatus === 'error') return 'bg-[#eb5757]/15 text-[#eb5757]';
